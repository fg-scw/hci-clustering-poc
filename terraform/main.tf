...
locals {
  ...
  # SSH public key - read from variable or file
...
resource "scaleway_vpc_public_gateway" "main" {
  ...
  zone            = var.zone
  ip_id           = scaleway_vpc_public_gateway_ip.gw_ip[0].id
  bastion_enabled = var.enable_ssh_bastion
  bastion_port    = var.bastion_port
  tags            = concat(local.common_tags, ["role:gateway"])
}

# DHCP configuration for the public network (used by VMs)
resource "scaleway_vpc_public_gateway_dhcp" "main" {
  ...
  # Reserve IPs for Proxmox nodes (they use static IPs)
  # DHCP will serve IPs from .100 onwards for VMs
  pool_low  = cidrhost(var.public_network_subnet, 100)
  pool_high = cidrhost(var.public_network_subnet, 250)
}

# Connect Gateway to Public Network with NAT
...
resource "local_file" "env_file" {
  ...
  content  = <<-EOF
    ...
    # Ceph Configuration
    OSD_DISKS="${join(" ", [for d in local.ceph_osd_disks : replace(d, "/dev/", "")])}"
    CEPH_POOL_VM="vm-storage"
    CEPH_POOL_CT="ct-storage"
    CEPH_REPLICA_SIZE="${local.ceph_pool_size}"
    CEPH_MIN_SIZE="${local.ceph_pool_min_size}"
...
#
# NOTE: Each server may have different VLAN IDs even on the same Private Network
# This is normal for Scaleway Elastic Metal - VLANs correspond to physical rack locations
all:
  children:
    proxmox:
      hosts:
%{for i, server in scaleway_baremetal_server.nodes~}
        ${local.node_names[i]}:
          ...
          # VLAN IDs specific to this server (may differ between servers)
          public_vlan: ${try(local.server_vlans[i].public_vlan[0], 0)}
          cluster_vlan: ${try(local.server_vlans[i].cluster_vlan[0], 0)}
          ceph_osd_devices:
%{for disk in local.ceph_osd_disks~}
            - ${disk}
%{endfor~}
%{endfor~}
      vars:
...
# Each server has its own public_vlan and cluster_vlan in host_vars
...
# =============================================================================
# SSH Configuration (for Ansible bootstrap)
# =============================================================================
...
public_gateway:
  enabled: true
  ip: "${scaleway_vpc_public_gateway_ip.gw_ip[0].address}"
  bastion_port: ${var.bastion_port}
  dhcp_pool_start: "${cidrhost(var.public_network_subnet, 100)}"
  dhcp_pool_end: "${cidrhost(var.public_network_subnet, 250)}"
%{else~}
public_gateway:
  enabled: false
%{endif~}
